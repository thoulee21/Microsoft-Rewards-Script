...
async function loopForAccountsConfirmation() {
    ...
    for (;;) {
        const ans = (
            await prompt(
                'Have you entered your passwords in accounts.json? (yes/no) : '
            )
        ).toLowerCase()
        if (['yes', 'y'].includes(ans)) break
        if (['no', 'n'].includes(ans)) {
            log(
                'Please enter your passwords in accounts.json and save the file (Ctrl+S), then answer yes.'
            )
            continue
        }
...
async function fullSetup() {
    ...
    await installPlaywrightBrowsers()
    const start = (
        await prompt('Do you want to start the program now? (yes/no) : ')
    ).toLowerCase()
...
async function installPlaywrightBrowsers() {
    const PLAYWRIGHT_MARKER = path.join(
        PROJECT_ROOT,
    ...
    try {
        ...
        warn(
            'Failed to install Playwright chromium automatically. You can manually run: npx playwright install chromium'
        )
    }
}
