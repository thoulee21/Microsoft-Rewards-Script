...
export default class BrowserFunc {
        ...
        try {
            ...
            for (let iteration = 1; iteration <= maxIterations; iteration++) {
                ...
                const isSuspended = await page
                    .waitForSelector('#suspendedAccountHeader', {
                        state: 'visible',
                        timeout: 2000,
                    })
                    .then(() => true)
                ...
                try {
                    ...
                    await page.waitForSelector('#more-activities', {
                        timeout: 1000,
                    })
                    this.bot.log(
                        this.bot.isMobile,
        ...
        try {
            ...
            if (currentURL.hostname !== dashboardURL.hostname) {
                this.bot.log(
                    this.bot.isMobile,
                    'DASHBOARD-DATA',
                    'Provided page did not equal dashboard page, redirecting to dashboard page'
                )
            ...
            if (!scriptContent) {
                throw this.bot.log(
                    this.bot.isMobile,
                    'GET-DASHBOARD-DATA',
                    'Dashboard data not found within script',
                    'error'
                )
            }

            // Extract the dashboard object from the script content
            const dashboardData = await this.bot.homePage.evaluate(
                (scriptContent) => {
            ...
            if (!dashboardData) {
                throw this.bot.log(
                    this.bot.isMobile,
                    'GET-DASHBOARD-DATA',
                    'Unable to parse dashboard script',
                    'error'
                )
            }
    ...
    async getBrowserEarnablePoints(): Promise<EarnablePoints> {
        try {
            ...
            if (data.userStatus.counters.pcSearch?.length) {
                data.userStatus.counters.pcSearch.forEach(
                    (x) =>
                        (desktopSearchPoints +=
                            x.pointProgressMax - x.pointProgress)
                )
            }

            // Mobile Search Points
            if (data.userStatus.counters.mobileSearch?.length) {
                data.userStatus.counters.mobileSearch.forEach(
                    (x) =>
                        (mobileSearchPoints +=
                            x.pointProgressMax - x.pointProgress)
                )
            ...
            // Daily Set
            data.dailySetPromotions[this.bot.utils.getFormattedDate()]?.forEach(
                (x) => (dailySetPoints += x.pointProgressMax - x.pointProgress)
            )
    ...
    async getAppEarnablePoints(accessToken: string) {
        try {
            ...
            const userDataResponse: AppUserData = (
                await this.bot.axios.request(userDataRequest)
            ).data
            const userData = userDataResponse.response
            const eligibleActivities = userData.promotions.filter((x) =>
            ...
            for (const item of eligibleActivities) {
                if (item.attributes.type === 'msnreadearn') {
                    points.readToEarn =
                        parseInt(item.attributes.pointmax ?? '') -
                        parseInt(item.attributes.pointprogress ?? '')
                    break
                } else if (item.attributes.type === 'checkin') {
                    const checkInDay =
                        parseInt(item.attributes.progress ?? '') % 7

                    if (
                        checkInDay < 6 &&
                        new Date().getDate() !=
                            new Date(
                                item.attributes.last_updated ?? ''
                            ).getDate()
                    ) {
                        points.checkIn = parseInt(
                            item.attributes[
                                'day_' + (checkInDay + 1) + '_points'
                            ] ?? ''
                        )
                    }
                    break
        ...
        try {
            ...
            const scriptContent = $('script')
                .filter((index, element) => {
                    return $(element)
                        .text()
                        .includes('_w.rewardsQuizRenderInfo')
                })
                .text()
            ...
            if (scriptContent) {
                ...
                if (match && match[1]) {
                    ...
                    throw this.bot.log(
                        this.bot.isMobile,
                        'GET-QUIZ-DATA',
                        'Quiz data not found within script',
                        'error'
                    )
                }
            } else {
                throw this.bot.log(
                    this.bot.isMobile,
                    'GET-QUIZ-DATA',
                    'Script containing quiz data not found',
                    'error'
                )
            }
        } catch (error) {
    ...
    async waitForQuizRefresh(page: Page): Promise<boolean> {
        try {
            await page.waitForSelector('span.rqMCredits', {
                state: 'visible',
                timeout: 10000,
            })
            await this.bot.utils.wait(2000)
    ...
    async checkQuizCompleted(page: Page): Promise<boolean> {
        try {
            await page.waitForSelector('#quizCompleteContainer', {
                state: 'visible',
                timeout: 2000,
            })
            await this.bot.utils.wait(2000)
    ...
    async getPunchCardActivity(
        page: Page,
        activity: PromotionalItem | MorePromotion
    ): Promise<string> {
        let selector = ''
        try {
            const html = await page.content()
            ...
            const element = $('.offer-cta')
                .toArray()
                .find((x) => x.attribs.href?.includes(activity.offerId))
            if (element) {
