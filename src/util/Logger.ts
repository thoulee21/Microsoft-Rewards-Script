...
export function log(
    ...
    if (
        configData.logExcludeFunc.some(
            (x) => x.toLowerCase() === title.toLowerCase()
        )
    ) {
        return
    }
    ...
    const platformText =
        isMobile === 'main' ? 'MAIN' : isMobile ? 'MOBILE' : 'DESKTOP'
    const chalkedPlatform =
        isMobile === 'main'
            ? chalk.bgCyan('MAIN')
            : isMobile
              ? chalk.bgBlue('MOBILE')
              : chalk.bgMagenta('DESKTOP')
    ...
    if (
        !configData.webhookLogExcludeFunc.some(
            (x) => x.toLowerCase() === title.toLowerCase()
        )
    ) {
    ...
    const applyChalk =
        color && typeof chalk[color] === 'function'
            ? (chalk[color] as (msg: string) => string)
            : null
