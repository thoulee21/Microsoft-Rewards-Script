...
export function getSystemComponents(mobile: boolean): string {
    ...
    const uaPlatform: string = mobile
        ? `Android 1${Math.floor(Math.random() * 5)}`
        : 'Win64; x64'
...
export async function getAppComponents(isMobile: boolean) {
    ...
    const edgeVersion = isMobile
        ? versions.android
        : (versions.windows as string)
...
export async function updateFingerprintUserAgent(
    ...
    try {
        ...
        fingerprint.fingerprint.navigator.userAgentData =
