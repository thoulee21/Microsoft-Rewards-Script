                ...
                if (quizData.numberOfOptions === 8) {
                    ...
                    for (let i = 0; i < quizData.numberOfOptions; i++) {
                        ...
                        const answerAttribute = await answerSelector?.evaluate(
                            (el) => el.getAttribute('iscorrectoption')
                        )

                        if (
                            answerAttribute &&
                            answerAttribute.toLowerCase() === 'true'
                        ) {
                    ...
                    for (let i = 0; i < quizData.numberOfOptions; i++) {
                        ...
                        const dataOption = await answerSelector?.evaluate(
                        ...
                        if (dataOption === correctOption) {
                            ...
                            const refreshSuccess =
                                await this.bot.browser.func.waitForQuizRefresh(
                                    page
                                )
                            if (!refreshSuccess) {
                                await page.close()
