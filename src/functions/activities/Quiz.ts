...
export class Quiz extends Workers {
  async doQuiz(page: Page) {
    ...
    try {
      ...
      const quizNotStarted = await page
        .waitForSelector("#rqStartQuiz", { state: "visible", timeout: 2000 })
        .then(() => true)
        .catch(() => false);
      if (quizNotStarted) {
      ...
      for (let question = 0; question < questionsRemaining; question++) {
        if (quizData.numberOfOptions === 8) {
          ...
          for (let i = 0; i < quizData.numberOfOptions; i++) {
            ...
            const answerAttribute = await answerSelector?.evaluate((el) =>
              el.getAttribute("iscorrectoption"),
            );
          ...
          for (let i = 0; i < quizData.numberOfOptions; i++) {
            ...
            const dataOption = await answerSelector?.evaluate((el) =>
