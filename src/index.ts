...
export class MicrosoftRewardsBot {
    ...
    private async runTasks(accounts: Account[]) {
        for (const account of accounts) {
            log(
                'main',
                'MAIN-WORKER',
                `Started tasks for account ${account.email}`
            )
            ...
            if (this.config.parallel) {
                ...
                const desktopPromise = this.Desktop(account).catch((e) => {
                    log(
                        false,
                        'TASK',
                        `Desktop flow failed early for ${account.email}: ${
                ...
                const mobilePromise = mobileInstance
                    ...
                    .catch((e) => {
                        log(
                            true,
                            'TASK',
                            `Mobile flow failed early for ${account.email}: ${
                                e instanceof Error ? e.message : e
                            }`,
                            'error'
                        )
                        errors.push(formatFullErr('mobile', e))
                        return null
                    })
            ...
            log(
                'main',
                'MAIN-WORKER',
                `Completed tasks for account ${account.email}`,
                'log',
                'green'
            )
        }
        ...
        log(
            this.isMobile,
            'MAIN-PRIMARY',
            'Completed tasks for ALL accounts',
            'log',
            'green'
        )
        ...
        if (process.env.DEBUG_REWARDS_VERBOSE === '1') {
            for (const summary of this.accountSummaries) {
                log(
                    'main',
                    'SUMMARY-DEBUG',
                    `Account ${summary.email} collected D:${
                        summary.desktopCollected
                    } M:${summary.mobileCollected} TOTAL:${
    ...
    async Desktop(account: Account) {
        ...
        log(
            this.isMobile,
            'MAIN-POINTS',
            `Current point count: ${this.pointsInitial}`
        )

        const browserEnarablePoints =
            await this.browser.func.getBrowserEarnablePoints()

        // Tally all the desktop points
        this.pointsCanCollect =
            browserEnarablePoints.dailySetPoints +
        ...
        log(
            this.isMobile,
            'MAIN-POINTS',
            `You can earn ${this.pointsCanCollect} points today`
        )
        ...
        const after = await this.browser.func
            .getCurrentPoints()
            .catch(() => initial)
        // Close desktop browser
        await this.browser.func.closeBrowser(browser, account.email)
    ...
    async Mobile(account: Account) {
        ...
        // Login into MS Rewards, then go to rewards homepage
        await this.login.login(this.homePage, account.email, account.password)
        this.accessToken = await this.login.getMobileAccessToken(
            this.homePage,
            account.email
        )
        ...
        const data = await this.browser.func.getDashboardData()
        const initialPoints =
            data.userStatus.availablePoints || this.pointsInitial || 0

        const browserEnarablePoints =
            await this.browser.func.getBrowserEarnablePoints()
        const appEarnablePoints = await this.browser.func.getAppEarnablePoints(
            this.accessToken
        )

        this.pointsCanCollect =
            browserEnarablePoints.mobileSearchPoints +
            appEarnablePoints.totalEarnablePoints

        log(
            this.isMobile,
            'MAIN-POINTS',
            `You can earn ${this.pointsCanCollect} points today (Browser: ${browserEnarablePoints.mobileSearchPoints} points, App: ${appEarnablePoints.totalEarnablePoints} points)`
        )
        ...
        if (this.config.workers.doMobileSearch) {
            ...
            if (data.userStatus.counters.mobileSearch) {
                ...
                const mobileSearchPoints = (
                    await this.browser.func.getSearchPoints()
                ).mobileSearch?.[0]

                if (
                    mobileSearchPoints &&
                    mobileSearchPoints.pointProgressMax -
                        mobileSearchPoints.pointProgress >
                        0
                ) {
                ...
                if (
                    ...
                    log(
                        this.isMobile,
                        'MAIN',
                        `Max retry limit of ${this.config.searchSettings.retryMobileSearchAmount} reached. Exiting retry loop`,
                        'warn'
                    )
                    ...
                    log(
                        this.isMobile,
                        'MAIN',
                        `Attempt ${this.mobileRetryAttempts}/${this.config.searchSettings.retryMobileSearchAmount}: Unable to complete mobile searches, bad User-Agent? Increase search delay? Retrying...`,
                        'log',
                        'yellow'
                    )
                ...
                log(
                    this.isMobile,
                    'MAIN',
                    'Unable to fetch search points, your account is most likely too "new" for this! Try again later!',
                    'warn'
                )
            }
        }
        ...
        log(
            ...
            `The script collected ${
                afterPointAmount - initialPoints
            } points today`
        )
